...
#define COLOR_DEV		SILVER
#define COLOR_NUM		BLACK
...
void plotTradeProfile(int bars)
{
	...
	if(is(EXITRUN))
	{
		...
		for(all_trades) if(TradeIsClosed) { // calculate minimum & maximum profit in pips
			vWinMax = max(vWinMax,toPIP(TradeProfit));
			vLossMax = max(vLossMax,-toPIP(TradeProfit));
		}
