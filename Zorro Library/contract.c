...
void contractPrintChain(int Handle, int To)
{
	set(LOGFILE);
...
void contractPrintChain() { contractPrintChain(0,TO_CSV); }
...
char* contractClass(CONTRACT* c)
{
	if (Live) return (char*)c;
	return (char*)"";
}
...
// https://www.interactivebrokers.com/en/index.php?f=26660&hm=us&ex=us
var comboMargin(int Sign, int AssetType)
{
	...
	case 8: // call spread (call-call), max(Strike Long - Strike Short,0)
		return abs(comboStrike(1)-comboStrike(2)) / 2;
...
void plotContract(int N,CONTRACT* C,var HistVol,var Min,var Max,int Days,int Modus)
{
	...
	string Title = (string)"At Expiry";
	int Type = LINE, Color = BLUE;
	if(Modus == 2) {
		Title = strf("At %i Days",Days); Color = RED;
	} else if(Modus == 3) {
		Title = (string)"Delta"; Type |= AXIS2; Color = GREEN;
	}
...
// N = 1 -> monthly, N = 3 -> quarterly
int contractSetSymbol(int Days,int N)
{
	int Year = year(0),Month = N;
	var MinDate = wdate(0)+Days;
	var	Date = nthDay(Year,Month,FRIDAY,3);
	while(Date < MinDate) {
		if((Month += N) > 12) { Month = N; Year++; }
		Date = nthDay(Year,Month,FRIDAY,3);
	}
	int Expiry = ymd(Date);
	string Fut = strstr(SymbolTrade,"FUT-");
	if(Fut) {
		memcpy(Fut+4,strf("%8i",Expiry),8);
