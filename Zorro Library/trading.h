...
typedef struct STAT { // statistics from optimization run
	float fPar,fObj;
	float fProfit,fLoss;
	float fNumWon,fNumLost;
} STAT;
...
#define TR_FRC			(1<<3)	// don't round to 1 PIP
#define TR_POS			(1<<4)	// check position before closing
#define TR_INVERSE		(1<<5)	// inverse contract
#define TR_EXTEND		(1<<6)	// extend lifetime
#define TR_ANYSTOP		(1<<7)	// move stops in any direction
#define TR_ANYLIMIT		(1<<8)	// move limits in any direction
...
// clear statistics until this position
	...
	int	nPad;
	var	vPad[2];
	var	vObjective;			// training objective
	var	vPerformance;	// User performance data
...
// clear again until this position
	...
	int	nCycles;			// number of WFO cycles per core
	int	nSteps[MAX_PARAMS];	// list of optimize steps
	float fParam[MAX_PARAMS];	// list of optimized parameters
	STAT* ParSteps;		// parameter/WFO optimization statistics
	char* ParNames;		// parameter names
	var* pCurve;		// component equity/balance curve, for future use
} STATUS;
...
// simulation/optimization parameters (r/w)
	...
	int	nButton;	// button number clicked
...
// simulation state (r/o)
	...
	uint nHash;			// Hash of vars that affect loading 
	int	pad62[3];
	...
	string *pAlgos; // user supplied null terminated list of Algos 
	...
	const char *sLogFolder;	// report output folder
	const char *pPad71;
	STAT* pWFOResults;
	STAT* pSampleResults;
	int	pad7[8];
...
// chart/log parameters (r/w)
	...
	int	flags2;		// internal flags
	int	flags4;		// internal switches
	int	flags3;		// internal flags
...
#define ASSETSET	(NF|(1<<17))	// asset() function called
...
#define NEWDAY		(NF|(1<<23))	// Day change after previous bar 
...
#define NOFLAT		FLAG(13) // prevent shifting series on flat bars
#define FLAG14		FLAG(14) // trail entry/exit limits at trade updates
#define FLAG15		FLAG(15) // unused
...
#define SHAPE	(1<<9)
#define PHANTOM	(1<<16) // TrainMode
#define LIMITS	(1<<17) // TrainMode
...
#define SOFTSWAP	(1<<22)	// a->flags, start accumulating swaps immediately
...
#define HEATMAP (2<<6)
#define SPLINE	(1<<28)
#define LABELS	(1<<29) // print heatmap labels
...
#define SET_RESTART		75 // set the HHMM time for restarting the API
