...
input double   InpADX_Level         = 15.0;        // ADX Minimum Level (DayTrade Default)
...
input double   InpTP_Mult           = 1.5;         // Take Profit ATR Multiplier
input double   InpSL_Mult           = 1.0;         // Stop Loss ATR Multiplier
input double   InpBE_Mult           = 1.0;         // Break Even ATR Multiplier
input double   InpMinATR_EURUSD     = 0.0004;      // Min ATR for EURUSD (DayTrade Default)
input double   InpMinATR_GBPUSD     = 0.0006;      // Min ATR for GBPUSD (DayTrade Default)
input double   InpMinDistanceATR    = 0.2;         // Min Distance from EMA50 in ATR
...
void OnTick()
{
   ...
   // Apply ArraySetAsSeries to all buffers per Patch instructions
   ...
   double ema21_1 = ema21[0]; // Buffer is series: index 0 is shift 1
   double ema50_1 = ema50[0];
   
   // Reduced Distance Filter Strictness
   double dist = MathAbs(close1 - ema50_1);

   // Continuation Entry Logic (Removed Crossover Requirement)
   // BUY Condition
   if(ema21_1 > ema50_1 && close1 > ema21_1 && adx[0] > InpADX_Level && pDI[0] > mDI[0] && dist > (InpMinDistanceATR * curATR))
   {
   ...
   // SELL Condition
   else if(ema21_1 < ema50_1 && close1 < ema21_1 && adx[0] > InpADX_Level && mDI[0] > pDI[0] && dist > (InpMinDistanceATR * curATR))
   {
...
bool ExecuteTrade(ENUM_ORDER_TYPE type, double price, double sl, double tp)
{
   ...
   if(!result) Print("Trade failed. Retcode: ", trade.ResultRetcode());
   return result;
}
