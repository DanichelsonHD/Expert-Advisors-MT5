...
input double InpATR_TakeMult     = 2.0;     // Take Profit ATR multiplier
input int    InpEMA_Fast_Period  = 21;      // Fast EMA period
input int    InpEMA_Slow_Period  = 50;      // Slow EMA period
input int    InpATR_Period       = 14;      // ATR period
input int    InpADX_Period       = 14;      // ADX period
input double InpADX_Minimum      = 20.0;    // ADX minimum value
input double InpATR_DistanceMult = 0.5;     // Distance from EMA50 multiplier
input bool   InpUseBreakEven     = true;    // Enable Break Even
input bool   InvertLogic         = false;   // Invert logic for opposite signals
...
int OnInit() {
   ...
   hEMA21 = iMA(_Symbol, _Period, InpEMA_Fast_Period, 0, MODE_EMA, PRICE_CLOSE);
   hEMA50 = iMA(_Symbol, _Period, InpEMA_Slow_Period, 0, MODE_EMA, PRICE_CLOSE);
...
void OnTick() {
   ...
   // 5. Common Filters
   bool adxFilter = (data.adx[1] > InpADX_Minimum);
   if(!adxFilter) return;

   double dist = MathAbs(rates[0].close - data.ema50[1]);
   bool distFilter = (dist > (InpATR_DistanceMult * data.atr[1]));
   if(!distFilter) return;

   // 6. BUY SETUP
   if(data.ema21[1] > data.ema50[1] && data.plusDI[1] > data.minusDI[1]) {
      if(rates[0].close > data.ema50[1] && rates[1].close < data.ema21[2] && rates[0].close > data.ema21[1]) {
         ENUM_ORDER_TYPE type = InvertLogic ? ORDER_TYPE_SELL : ORDER_TYPE_BUY;
         ExecuteMarketOrder(type, data.atr[1]);
      }
   }
...
bool CheckDailyLimits() {
   ...
   double limitMoney = balance * (InpDailyLimit / 100.0);
