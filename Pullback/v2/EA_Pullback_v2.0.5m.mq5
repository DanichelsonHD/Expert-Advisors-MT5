...
input int    InpMinFlexScore           = 1;   // Minimum flexible score required
...
input int    InpMomentumLookback       = 5;   // Number of candles to measure breakout momentum
...
void OnTick() {
   ...
   MqlRates rates[];
   int copy_count = InpMomentumLookback + 2;
   if(CopyRates(_Symbol, _Period, 0, copy_count, rates) < copy_count) return; 
   ...
   for(int i = 2; i <= InpMomentumLookback; i++)
   {
   ...
   bool momentumBuy  = (rates[1].close > highestHigh);
   bool momentumSell = (rates[1].close < lowestLow);
   ...
   // Hard Filters
   bool hardBuy =
       (emaF1 > emaS1) &&                  // Direction alignment
       (adx1 > InpADX_Minimum) &&          // Trend strength mandatory
       (c1 > emaS1) &&                     // Price above structure
       (MathAbs(c1 - emaS1) > (InpATR_DistanceMult * atr1)); // Minimum expansion

   bool hardSell =
       (emaF1 < emaS1) &&
       (adx1 > InpADX_Minimum) &&
       (c1 < emaS1) &&
       (MathAbs(c1 - emaS1) > (InpATR_DistanceMult * atr1));
   ...
   // Flexible Scores
   int buyFlexScore = 0;
   if(emaS1 > emaS5)              buyFlexScore++;  // Slope strength
   if(atr1 > atr10)               buyFlexScore++;  // Volatility expansion
   if(pDI1 > mDI1)                buyFlexScore++;  // DI confirmation
   if(c2 < emaF2 && c1 > emaF1)   buyFlexScore++;  // Pullback structure

   int sellFlexScore = 0;
   if(emaS1 < emaS5)              sellFlexScore++;
   if(atr1 > atr10)               sellFlexScore++;
   if(mDI1 > pDI1)                sellFlexScore++;
   if(c2 > emaF2 && c1 < emaF1)   sellFlexScore++;

   // Flex Score Validation
   bool allowBuy  = hardBuy  && momentumBuy  && (buyFlexScore  >= InpMinFlexScore);
   bool allowSell = hardSell && momentumSell && (sellFlexScore >= InpMinFlexScore);
