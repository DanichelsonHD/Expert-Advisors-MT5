...
void OnTick() {
   ...
   if(emaF1 > emaS1)         buyRegimeScore++;  // Direction alignment
   if(emaS1 > emaS5)         buyRegimeScore++;  // Slope strength
   if(atr1 > atr10)          buyRegimeScore++;  // Volatility expansion
   if(adx1 > InpADX_Minimum) buyRegimeScore++;  // Trend strength

   int sellRegimeScore = 0;
   if(emaF1 < emaS1)         sellRegimeScore++;
   if(emaS1 < emaS5)         sellRegimeScore++;
   if(atr1 > atr10)          sellRegimeScore++;
   if(adx1 > InpADX_Minimum) sellRegimeScore++;

   // Entry Scores
   int buyEntryScore = 0;
   if(c1 > emaS1)                          buyEntryScore++;
   if(pDI1 > mDI1)                         buyEntryScore++;
   if(c2 < emaF2 && c1 > emaF1)            buyEntryScore++;
   if(MathAbs(c1 - emaS1) > (InpATR_DistanceMult * atr1)) buyEntryScore++;

   int sellEntryScore = 0;
   if(c1 < emaS1)                          sellEntryScore++;
   if(mDI1 > pDI1)                         sellEntryScore++;
   if(c2 > emaF2 && c1 < emaF1)            sellEntryScore++;
   if(MathAbs(c1 - emaS1) > (InpATR_DistanceMult * atr1)) sellEntryScore++;

   // Decision Logic
   bool allowBuy  = (buyRegimeScore  >= InpMinRegimeScore) &&
                    (buyEntryScore   >= InpMinEntryScore);

   bool allowSell = (sellRegimeScore >= InpMinRegimeScore) &&
                    (sellEntryScore  >= InpMinEntryScore);

   // Execution
   if(allowBuy)
   {
      int totalScore = buyRegimeScore + buyEntryScore;
      double riskMultiplier = 1.0;

      if(totalScore >= 6)
         riskMultiplier = InpRiskMultiplierLevel3;
      else if(totalScore >= 5)
         riskMultiplier = InpRiskMultiplierLevel2;
   ...
   if(allowSell)
   {
      int totalScore = sellRegimeScore + sellEntryScore;
      double riskMultiplier = 1.0;
