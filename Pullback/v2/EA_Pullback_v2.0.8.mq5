...
input double InpDailyLimit = 1.0;
input double InpATR_StopMult = 1.2;
input double InpATR_TakeMult = 2.5;
input int InpEMA_Fast_Period = 21;
input int InpEMA_Slow_Period = 50;
input int InpATR_Period = 14;
input int InpADX_Period = 14;
input double InpATR_DistanceMult = 0.6;
input double InpADX_Minimum = 27.0;     // Stronger trend requirement
input bool InpUseBreakEven = true;
input double InpStrongRegimeRiskMultiplier = 1.5;
input int InpMinRegimeScore = 2;
input int InpMinEntryScore = 3;
input double InpRiskMultiplierLevel2 = 1.3;
input double InpRiskMultiplierLevel3 = 1.6;
input int InpMinFlexScore = 3;          // Higher confirmation requirement
input double InpFlexRiskLevel2 = 1.3;
input double InpFlexRiskLevel3 = 1.6;
input int InpRSI_Period = 14;
input double InpRSI_BuyLevel = 38.0;    // Deeper pullback for BUY
input double InpRSI_SellLevel = 62.0;   // Deeper pullback for SELL
input double InpRSI_MaxBuyLevel = 60.0;
input double InpRSI_MinSellLevel = 40.0;
...
void OnTick()
{
    ...
    bool hardBuy =
        ...
        (adx1 > InpADX_Minimum) &&
        (c1 > emaS1) &&
        (emaS1 > emaS5);   // Slow EMA slope confirmation
    ...
    if (allowBuy)
    {
        ...
        ExecuteMarketOrder(ORDER_TYPE_BUY, atr1, rates, rates[1].time, riskMultiplier);
    }
    ...
    if (allowSell)
    {
        ...
        ExecuteMarketOrder(ORDER_TYPE_SELL, atr1, rates, rates[1].time, riskMultiplier);
    }
}
...
void ExecuteMarketOrder(ENUM_ORDER_TYPE type, double atr, MqlRates &rates[], datetime candleTime, double riskMultiplier)
{
    ...
    double atrStop = InpATR_StopMult * atr;
    double slPrice;

    if (type == ORDER_TYPE_BUY)
    {
        double structuralLow = rates[2].low;
        slPrice = MathMin(structuralLow - (0.2 * atr),
                          price - atrStop);
    }
    else
    {
        double structuralHigh = rates[2].high;
        slPrice = MathMax(structuralHigh + (0.2 * atr),
                          price + atrStop);
    }
