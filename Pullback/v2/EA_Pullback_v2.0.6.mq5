...
void OnTick() {
   ...
   if(MathAbs(c1 - emaS1) > (InpATR_DistanceMult * atr1)) buyFlexScore++;  // Distance expansion
   if(rsi2 <= InpRSI_BuyLevel && rsi1 > rsi2 && rsi1 < InpRSI_MaxBuyLevel) buyFlexScore++; // RSI condition
   ...
   if(MathAbs(c1 - emaS1) > (InpATR_DistanceMult * atr1)) sellFlexScore++;
   if(rsi2 >= InpRSI_SellLevel && rsi1 < rsi2 && rsi1 > InpRSI_MinSellLevel) sellFlexScore++;
