...
void OnTick() {
   ...
   // Regime Filters
   bool buyRegime  = (emaF1 > emaS1) && (emaS1 > emaS5) && (adx1 > InpADX_Minimum);
   bool sellRegime = (emaF1 < emaS1) && (emaS1 < emaS5) && (adx1 > InpADX_Minimum);
      ...
      if(c1 > emaS1 && pDI1 > mDI1) {
         if(c2 < emaF2 && c1 > emaF1) {
            ...
            if(dist > (InpATR_DistanceMult * atr1)) {
               ExecuteMarketOrder(ORDER_TYPE_BUY, atr1, rates[1].time);
            }
         }
      }
   }
   ...
   if(sellRegime) {
      if(c1 < emaS1 && mDI1 > pDI1) {
         if(c2 > emaF2 && c1 < emaF1) {
            double dist = MathAbs(c1 - emaS1);
            if(dist > (InpATR_DistanceMult * atr1)) {
               ExecuteMarketOrder(ORDER_TYPE_SELL, atr1, rates[1].time);
            }
         }
      }
   }
...
void ExecuteMarketOrder(ENUM_ORDER_TYPE type, double atr, datetime candleTime) {
