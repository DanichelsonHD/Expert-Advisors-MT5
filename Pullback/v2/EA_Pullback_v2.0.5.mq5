...
input double InpRSI_BuyLevel      = 45.0; // RSI pullback threshold for BUY
input double InpRSI_SellLevel     = 55.0; // RSI pullback threshold for SELL
...
void OnTick() {
   ...
   // RSI Logic
   bool rsiBuy =
       (rsi2 <= InpRSI_BuyLevel) &&
       (rsi1 > rsi2) &&
       (rsi1 < InpRSI_MaxBuyLevel);

   bool rsiSell =
       (rsi2 >= InpRSI_SellLevel) &&
       (rsi1 < rsi2) &&
       (rsi1 > InpRSI_MinSellLevel);
   ...
   // Validation
   bool allowBuy  = hardBuy  && rsiBuy  && (buyFlexScore  >= InpMinFlexScore);
   bool allowSell = hardSell && rsiSell && (sellFlexScore >= InpMinFlexScore);
...
bool FillData(IndicatorData &d) {
   ...
   if(CopyBuffer(hRSI, 0, 0, 3, d.rsi) < 3) return false;
