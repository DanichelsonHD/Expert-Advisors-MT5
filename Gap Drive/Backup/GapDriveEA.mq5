...
void ExecuteTrade(const int setupIdx)
{
   ...
   if(g_setups[setupIdx].bullish_gap)
      result = g_trade.Buy(InpLotSize, _Symbol, 0.0, sl, tp, "GapDriveEA");
   else
      result = g_trade.Sell(InpLotSize, _Symbol, 0.0, sl, tp, "GapDriveEA");

   if(result)
   {
      g_setups[setupIdx].position_ticket = g_trade.ResultDeal();
      PrintFormat("GapDriveEA: Trade opened | %s | Ticket=%llu | SL=%.5f | TP=%.5f",
