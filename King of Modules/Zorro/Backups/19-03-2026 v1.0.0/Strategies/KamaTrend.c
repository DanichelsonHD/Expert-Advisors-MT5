// ============================================================
//  Strategies/KamaTrend.c
//  Zorro translation of Strategies/KamaTrend.mqh
...
//
//  GetKamaFlip: reads KAMA_FLIP_LOOKBACK+1 = 4 color values
//    from shift=1 (bars 1..4). Detects any flip within last 3 bars.
//    MQL5: CopyBuffer(g_handleKAMA,1,shift=1,count=4,buf)
//    Zorro: kamaColorSeries()[1..KAMA_FLIP_LOOKBACK+1]
//    Index mapping: buf[0]=bar1, buf[1]=bar2, buf[2]=bar3, buf[3]=bar4
//      (MQL5 CopyBuffer fills in ascending time order when offset=1)
//    In Zorro series: [1]=bar1(newest), [N]=barN(oldest)
//    Loop rewritten accordingly (see below).
//
//  KAMA color flip mapping (Indicators.c convention 0/1):
//    MQL5: previous==1(bearish)&&current==2(bullish) → bullishFlip
//    Zorro: previous==1(bearish)&&current==0(bullish) → bullishFlip
//    MQL5: previous==2(bullish)&&current==1(bearish) → bearishFlip
//    Zorro: previous==0(bullish)&&current==1(bearish) → bearishFlip
//
//  PassATRExpansionFilter:
//    iATR static handle (ATR50) → series(ATR(50))[0]/[1]
//    atrFast[0]=current, atrFast[1]=prev → series(ATR(14))[0]/[1]
//    ATR(14) series must share the same call-site as GetATR().
//    [NOTE] Zorro series() does NOT share buffers across call-sites.
//    To access [0] and [1] safely, we use a named series pointer.
//
//  PassADXTrendFilter:
//    iADX static handle → series(ADX(InpADXPeriod))[0]/[1]
//    ADX(period) is a built-in Zorro function returning the main line.
//    ADX filter logic: adxNow > adxPrev AND adxNow < InpADXComparision
//    (original comment shows multiple threshold experiments; the
//    implemented condition is `adxNow < InpADXComparision` — preserved).
// ============================================================
