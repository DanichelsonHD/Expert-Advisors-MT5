...
void optimizeCalls()
{
    ...
    Final_Target = 300 / 20.0 * PIP;

    Stop_Lookback = 4;
    Stop_Buffer   = 130 / 10.0 * PIP;
}
...
void manageTrades()
{
    // Proper way to iterate through open trades in Zorro
    for(listedTrade(ANY))
    {
        ...
        // We use [0] for immediate reaction
        bool overbought = rsiSeries[0] >= RSI_Overbought || closePrices[0] >= bbUpper[0];
        bool oversold   = rsiSeries[0] <= RSI_Oversold   || closePrices[0] <= bbLower[0];

        // LONG EXIT: Exit if market becomes overbought
        if (isLong && overbought)
        {
            exitTrade(tr); 
            continue; 
        }

        // SHORT EXIT: Exit if market becomes oversold
        if (isShort && oversold)
        {
            exitTrade(tr);
            continue;
        ...
        // FAILSAFE: 40 bars limit
        if (Bar - tr->nBarOpen > 40)
            exitTrade(tr);
    }
}
...
DLLFUNC void run()
{
    ...
    if (entryLong() && NumOpenLong == 0)
    {
        ...
        TakeProfit = 0;//priceClose(0) + 10 * ATR(14);
