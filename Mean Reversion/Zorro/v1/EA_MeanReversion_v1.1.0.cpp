...
// =============================================================================
// Entry Logic — Immediate 2-bar confirmation
// =============================================================================

// ---- LONG ----
bool entryLong()
{
    var bbLow_prev = bbLower[1];
    var bbLow_now  = bbLower[0];

    bool wasOversold =
        rsiSeries[1] < RSI_Oversold &&
        closePrices[1] < bbLow_prev;

    bool confirmation =
        rsiSeries[0] > RSI_Oversold &&
        closePrices[0] > bbLow_now;

    return wasOversold && confirmation && filtersPass();
}

// ---- SHORT ----
bool entryShort()
{
    var bbHigh_prev = bbUpper[1];
    var bbHigh_now  = bbUpper[0];

    bool wasOverbought =
        rsiSeries[1] > RSI_Overbought &&
        closePrices[1] > bbHigh_prev;

    bool confirmation =
        rsiSeries[0] < RSI_Overbought &&
        closePrices[0] < bbHigh_now;

    return wasOverbought && confirmation && filtersPass();
}
...
void manageTrades()
{
    ...
    while(tr = forTrade(0)) // 0 = todos os trades abertos
    {
        ...
        {
            if (priceClose(0) < smaSeries[0] - 1.5 * atrSeries[0])
            {
                exitTrade(tr);
                continue;
            }
