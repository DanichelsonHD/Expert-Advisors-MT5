...
input bool               InpUseKAMAStrongTrendFilter = false; // Enable KAMA Strong Trend Filter
input int                InpKAMASlopeLookback        = 5;     // Bars to measure slope
input double             InpKAMASlopeATRMultiplier   = 1.5;   // Slope threshold in ATR units
...
//--- Fetch KAMA main buffer: 1 + InpKAMASlopeLookback values from bar 1
//    [0]=oldest, [InpKAMASlopeLookback]=bar1
   double kamaBuf[];
   ...
   if(CopyBuffer(g_handleKAMA, 0, 1, InpKAMASlopeLookback + 1, kamaBuf) < InpKAMASlopeLookback + 1)
     {
      Print("WARN: CopyBuffer KAMA main failed.");
      return;
     }
...
//--- Derive position state from platform via CPositionInfo
   ...
   if(InpUseKAMAStrongTrendFilter)
     {
      double kamaSlope     = MathAbs(kamaBuf[InpKAMASlopeLookback] - kamaBuf[0]);
      double slopeThreshold = currentATR * InpKAMASlopeATRMultiplier;
      if(kamaSlope > slopeThreshold)
        {
         Print("INFO: Strong trend filter blocked entry. Slope=", kamaSlope,
