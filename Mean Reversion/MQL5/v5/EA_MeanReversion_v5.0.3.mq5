...
bool IsBBCompressed(double upper, double lower)
  {
   if(!InpUseBBCompression) return false;

   double width = GetBBWidthPoints(upper, lower);

   return (width < InpBBMinWidthPoints);
  }
   ...
   double points)
{
   ...
   if(mode == TAKE_RSI)
   {
      if(isLong && rsi > InpRSIOverbought)
         return entryPrice; // force exit via BE-style TP

      if(!isLong && rsi < InpRSIOversold)
         return entryPrice;
   }
...
void ProcessEntries(ENUM_ENTRY_STATE &state,
                    ...
                    const bool isBear)
  {
   bool isCompressed = IsBBCompressed(upper, lower);

   bool normalLong = ShouldEnterLong(state, rsi, price, lower);
   bool normalShort = ShouldEnterShort(state, rsi, price, upper);

   // --- BB compression inversion ---
   if(isCompressed)
     {
      // invert signals
      bool invertedLong  = normalShort;
      bool invertedShort = normalLong;

      normalLong  = invertedLong;
      normalShort = invertedShort;
     }
