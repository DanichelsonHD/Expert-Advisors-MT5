...
//| Constants                                                        |
//+------------------------------------------------------------------+
static const int MAX_STATE_BARS = 3;
...
void OnTick()
  {
   ...
   double rsiBuf[1], upperBuf[1], lowerBuf[1], smaBuf[2];
   double highBuf[InpStopLookback], lowBuf[InpStopLookback];

   if(CopyBuffer(g_handleRSI, 0, 1, 1, rsiBuf)  < 1) { Print("WARN: RSI copy failed.");      return; }
   if(CopyBuffer(g_handleBB,  1, 1, 1, upperBuf) < 1) { Print("WARN: BB upper copy failed."); return; }
   if(CopyBuffer(g_handleBB,  2, 1, 1, lowerBuf) < 1) { Print("WARN: BB lower copy failed."); return; }
   if(CopyBuffer(g_handleSMA, 0, 1, 2, smaBuf)   < 2) { Print("WARN: SMA copy failed.");      return; }
   if(CopyHigh(_Symbol, _Period, 1, InpStopLookback, highBuf) < InpStopLookback)
     { Print("WARN: High copy failed."); return; }
   if(CopyLow(_Symbol, _Period,  1, InpStopLookback, lowBuf)  < InpStopLookback)
     { Print("WARN: Low copy failed.");  return; }
   ...
   // --- 7. Process exits FIRST ---
   ProcessExits(oppSignal);

   // --- 8. Process entries AFTER ---
