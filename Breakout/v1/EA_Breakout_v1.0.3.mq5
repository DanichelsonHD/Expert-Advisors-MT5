#property copyright "Structural Trendline Breakout EA v1.03"
#property link      ""
...
input bool            InpUseHTFConfirm          = true;       //Use HTF Confirm (Hard Filter)
                       ...
                       const double atrAvg)
{
    ...
    if (atrCurrent > atrAvg * 1.1) score++;
...
//=============================================================
// PROCESS DIRECTION (unified breakout evaluation)
//=============================================================
bool ProcessDirection(const int direction,
                      ...
                      const double adxVal)
{
    ...
    if (!CheckHardFilters(isBuy, rates, trendVal, atrCurrent, atrAvg, adxVal))
        return false;

    int flexScore = CalculateFlexScore(isBuy, rates, atrCurrent, atrAvg);
...
bool CheckBreakout(const double atrAvg)
{
    ...
    {
        int currentShift = 1;
        if ((g_lastLossBar - currentShift) < InpReEntryCandles)
            return false;
    }
    ...
    if (SwingHighCount >= 2)
    {
        ...
        {
            if (ProcessDirection(1, ptA, ptB, atrAvg, rates, atrCurrent, adxVal))
                return true;
        }
    }
...
bool ManageDailyLimits()
{
    ...
    for (uint i = 0; i < total; i++)
    {
        ...
        DealInfo d;
        d.time   = (datetime)HistoryDealGetInteger(ticket, DEAL_TIME);
    ...
    int dealCount = ArraySize(deals);
    for (int a = 0; a < dealCount - 1; a++)
    {
        for (int b = a + 1; b < dealCount; b++)
        {
            if (deals[b].time < deals[a].time)
            {
                DealInfo tmp = deals[a];
                deals[a]     = deals[b];
                deals[b]     = tmp;
            }
        }
