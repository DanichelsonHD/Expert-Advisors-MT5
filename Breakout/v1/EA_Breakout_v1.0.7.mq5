#property copyright "Structural Trendline Breakout EA v1.07"
#property link      ""
...
input int             InpReEntryCandles         = 8;          //Min Re-Entry Candles
...
input int             InpMinFlexScore           = 3;          //Min Flex Score
...
input double          InpADXMin                 = 20.0;       //ADX Minimum Threshold
input double          InpCandleBodyRatio        = 0.6;        //Candle Body Ratio
input int             InpMagicNumber            = 20250101;   //Magic Number
input double          InpSlopeMinFactor         = 0.02;       //Minimum Slope Strength (ATR factor)
input double          InpCompressionATRFactor   = 2.2;        //Max Compression Range in ATR
input double          InpImpulseATRFactor       = 0.3;        //Minimum Breakout Body Size in ATR
input double          InpATRExpansionRatio      = 1.05;       //ATR Expansion Ratio
input bool            InpRequireADXIncreasing   = true;       //Require Rising ADX
input bool            InpUseStochasticFlex      = true;       //Use Stochastic as Flex
...
bool ValidateTrendline(const SwingPoint &ptA,
                       ...
                       const double atrAvg)
{
    ...
    double priceDiff = MathAbs(ptA.price - ptB.price);
    if (priceDiff < atrAvg * 0.8)
        return false;

    double slope = (ptB.price - ptA.price) / double(ptB.index - ptA.index);
                       ...
                       const double adxVal)
{
    ...
    // +1 ADX Rising
    if (InpRequireADXIncreasing)
    {
        if (s_adxBuf[1] > s_adxBuf[2])
            score++;
    }
    ...
    // +1 Impulse Body vs ATR
    double body = MathAbs(s_rates[1].close - s_rates[1].open);
    if (body > atrCurrent * InpImpulseATRFactor)
        score++;

    // +1 Slope Strength
    double slope = (g_trendlineB_price - g_trendlineA_price) /
                   double(g_trendlineB_idx - g_trendlineA_idx);
    if (MathAbs(slope) > atrAvg * 0.02)
        score++;
    ...
    double compressionRange = highest - lowest;
    if (compressionRange <= atrAvg * InpCompressionATRFactor)
        score++;

    // +1 HTF alignment (flex only)
    if (InpUseHTFConfirm && !InpHTFAsHardFilter)
    {
        ...
        {
            if (isBuy  && s_rates[1].close > emaBuf[0]) score++;
            if (!isBuy && s_rates[1].close < emaBuf[0]) score++;
        }
    }
    ...
    // +1 Stochastic Momentum Alignment
    if (InpUseStochasticFlex)
    {
        double stochMain[];
        ArraySetAsSeries(stochMain, true);
        if (CopyBuffer(hStoch, 0, 0, 1, stochMain) >= 1)
        {
            if (isBuy  && stochMain[0] > InpStochBuyLevel)  score++;
            if (!isBuy && stochMain[0] < InpStochSellLevel) score++;
        }
    }
                       ...
                       const double adxVal)
{
    ...
    if (!CheckHardFilters(isBuy, trendVal, atrAvg))
        return false;

    int flexScore = CalculateFlexScore(isBuy, atrCurrent, atrAvg, trendVal, adxVal);
...
bool CheckBreakout(const double atrAvg)
{
    ...
    if (g_lastLossTime > 0)
    {
        int lastLossShift = iBarShift(_Symbol, InpTimeframe, g_lastLossTime, false);
        if (lastLossShift >= 0)
        {
    ...
    if (SwingHighCount >= 2)
    {
        ...
        {
            if (EvaluateDirection(1, ptA, ptB, atrAvg, atrCurrent, adxVal))
            {
    ...
    if (isBuy)
    {
        for (int i = SwingLowCount - 1; i >= 0; i--)
        {
            if (ValidSwingLows[i].price < entryPrice)
                return MathMax(ValidSwingLows[i].price, entryPrice - atrStop);
        }
    ...
    else
    {
        for (int i = SwingHighCount - 1; i >= 0; i--)
        {
            if (ValidSwingHighs[i].price > entryPrice)
                return MathMin(ValidSwingHighs[i].price, entryPrice + atrStop);
        }
        return entryPrice + atrStop;
    }
}
